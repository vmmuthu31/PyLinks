...
export default function Dashboard() {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/api/merchants/update-wallet`,
      ...
      console.error(err);
      alert(`Failed to connect wallet: ${err.response?.data?.error || err.message}`);
    } finally {
      setConnecting(false);
    }
  };
  ...
  const regenerateKey = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/merchants/regenerate-keys`,
        {},
  ...
  const generatePaymentQR = () => {
    ...
    const payUrl = `${import.meta.env.VITE_FRONTEND_URL || "http://localhost:5173"}/pay?merchantId=${merchantId}&amount=${amount}&memo=${encodeURIComponent(
      memo || ""
    )}`;
    setQrUrl(payUrl);
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-12 px-4">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-lg p-8 transition-all">
        ...
        <div className="space-y-3 mb-8 text-gray-700">
          <p>
            <span className="font-medium text-gray-900">Name:</span> {merchant.name}
          </p>
          <p>
