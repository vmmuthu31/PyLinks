...
export default function PayPage() {
    ...
    const fetchMerchant = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/api/merchants/${merchantId}`
        );
  ...
  const handlePayment = async () => {
    ...
    console.log("Sender:", account);
      ...
      const tx = await contract.transfer(merchant.walletAddress, parsedAmount, { gasLimit: 100000 } );
      await tx.wait();
      ...
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/payments/record`, {
        merchantId,
        txHash: tx.hash,
        amount,
        userWallet: account,
