...
async function verifyPayment({
  ...
  const provider = new import_ethers.ethers.providers.JsonRpcProvider(
    process.env.ETH_RPC_SEPOLIA
  );
  const contract = new import_ethers.ethers.Contract(PYUSD_CONTRACT, PYUSD_abi_default, provider);
  ...
  for (const event of events) {
    if (event.args && event.args.value.toString() === import_ethers.ethers.utils.parseUnits(amount.toString(), 6).toString()) {
      return {
        status: "paid",
...
function generateQRCodePayload(payload) {
  return import_qrcode.default.toDataURL(JSON.stringify(payload));
}
